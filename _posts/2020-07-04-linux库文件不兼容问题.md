...
这两天在写论文，看到有人推荐writefull这个工具，于是就想下载来用用。但启动应用后又遇到了pango不兼容的问题。第一反应还是去package page看看有没有解决办法，总不能再去降级pango吧。但遗憾的是只看到有人遇到了同样的问题，却没人给出现成的解决办法。既然没有针对writefull这一bug的现成办法，那就去找找看别人都是怎么解决pango库文件不兼容的问题。幸运的是搜到了其他软件也因为pango更新而出现异常的问题。于是依葫芦画瓢再一次顺利解决了问题。
